...
func (db *DB) Instance(ctx context.Context) *gorm.DB {
	if instance, ok := ctx.Value(ContextKeyDatabase).(*gorm.DB); ok {
		instance = otgorm.SetSpanToGorm(ctx, instance)
		return instance
	}

	db.instance = otgorm.SetSpanToGorm(ctx, db.instance)
	return db.instance
}

// ForceInstance returns underlying gorm db instance.
// Even if the transaction in progress, it'll return the underlying gorm db instance.
func (db *DB) ForceInstance(ctx context.Context) *gorm.DB {
	db.instance = otgorm.SetSpanToGorm(ctx, db.instance)
	return db.instance
}

// Ping checks if the connection is active
// or the database is reachable
